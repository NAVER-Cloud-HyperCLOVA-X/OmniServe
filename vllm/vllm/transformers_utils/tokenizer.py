...
def init_tokenizer_from_configs(model_config: ModelConfig):
    ...
    # Add placeholder tokens for multimodal embedding injection if enable_prompt_embeds is set
    if model_config.enable_prompt_embeds:
        multimodal_tokens = [
            "<|ptoken|>",  # Legacy vision token
            "<|DISCRETE_IMAGE_PAD|>",
            "<|IMAGE_PAD|>",
            "<|DISCRETE_AUDIO_PAD|>",
            "<|AUDIO_PAD|>",
        ...
        tokens_to_add = [t for t in multimodal_tokens if t not in vocab]
        ...
        if tokens_to_add:
            ...
            if num_added > 0:
                for token in tokens_to_add:
                    logger.info(
                        "Added placeholder token '%s' for multimodal embedding injection "
                        "(token_id=%d)",
                        token,
