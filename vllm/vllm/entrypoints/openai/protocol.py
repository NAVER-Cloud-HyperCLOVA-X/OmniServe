...
class CompletionRequest(OpenAIBaseModel):
    ...
    vision_embeddings: list[list[float]] | None = Field(
        default=None,
        description=(
            "External embeddings to inject at placeholder token positions. "
            "When provided along with a prompt containing placeholder tokens "
            "(e.g., <|ptoken|>), these embeddings will replace the placeholder "
            "token embeddings. The shape should be (num_placeholder_tokens, hidden_size)."
        ),
    )
    vision_embeddings_s3_path: str | None = Field(
        default=None,
        description=(
            "S3 path to the vision embeddings file (.pt format). "
            "Format: 'bucket_name/path/to/embeddings.pt'. "
            "The file should contain a torch tensor of shape (num_placeholder_tokens, hidden_size). "
            "This is mutually exclusive with vision_embeddings and vision_embeddings_url."
        ),
    )
    vision_embeddings_url: str | None = Field(
        default=None,
        description=(
            "Presigned URL to download the vision embeddings file (.pt format). "
            "The file should contain a torch tensor of shape (num_placeholder_tokens, hidden_size). "
            "This is mutually exclusive with vision_embeddings and vision_embeddings_s3_path."
        ),
    )
    # Multi-modal embeddings with discrete and continuous components
    multimodal_embeddings_s3_path: str | None = Field(
        default=None,
        description=(
            "S3 path to multi-modal embeddings file (.pt format). "
            "The file should contain a dict with 'discrete' (token IDs), "
            "'continuous' (embeddings), and optionally 'meta' fields. "
            "Use with placeholders: <|DISCRETE_IMAGE_PAD|> for discrete, "
            "<|IMAGE_PAD|> for continuous."
        ),
    )
    multimodal_embeddings_url: str | None = Field(
        default=None,
        description=(
            "Presigned URL to multi-modal embeddings file (.pt format). "
            "Same format as multimodal_embeddings_s3_path."
        ),
    )
    add_special_tokens: bool = Field(
        default=True,
...
class ExtractedToolCallInformation(BaseModel):
    ...
    # Omni: 순서가 보존된 content parts
    # [{"type": "text", "text": "..."}, {"type": "tool_call_ref", "tool_call_id": "..."}]
...
class ChatMessage(OpenAIBaseModel):
    ...
    # Omni multimodal content parts (순서 보존)
    # 예: [{"type": "text", "text": "..."}, {"type": "tool_call_ref", "tool_call_id": "..."}]
    content_parts: list[dict] | None = None
